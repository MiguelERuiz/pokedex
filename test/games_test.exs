defmodule GamesTest do
  ...
  test "get generation by numeric id" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.generation(1)
    assert status == 200
    assert body["main_region"]["name"] == "kanto"
  end
  ...
  test "get generation by atom name" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.generation(:generation_i)
    assert status == 200
    assert body["main_region"]["name"] == "kanto"
  end

  test "get generation by string name" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.generation("generation-i")
    assert status == 200
    assert body["main_region"]["name"] == "kanto"
  end

  test "get pokedex by numeric id" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.pokedex(1)
    assert status == 200
  ...
  test "get pokedex by atom name" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.pokedex(:national)
    assert status == 200
    assert body["name"] == "national"
  end
  ...
  test "get version by numeric id" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.version(1)
    assert status == 200
  ...
  test "get version by atom name" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.version(:red)
    assert status == 200
    assert body["name"] == "red"
  end
  ...
  test "get version group by numeric id" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.version_group(1)
    assert status == 200
    assert body["name"] == "red-blue"
  end
  ...
  test "get version group by atom name" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.version_group(:red_blue)
    assert status == 200
    assert body["name"] == "red-blue"
  end

  test "get version group by string name" do
    assert {:ok, %Tesla.Env{status: status, body: body}} = Pokedex.Games.version_group("red-blue")
    assert status == 200
    assert body["name"] == "red-blue"
  end
